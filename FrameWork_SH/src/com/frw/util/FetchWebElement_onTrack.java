...
public class FetchWebElement_onTrack extends Base{
	...
	public static  WebElement waitForElement_old(WebDriver driver,String objectlocatorType,String objectlocator, Integer waitSeconds ) {
		...
		try{
			...
			field=getElementByLocator(newById, driver);
	...
	//https://github.com/djangofan/WebDriverTestingTemplate/blob/master/commonlib/src/main/java/qa/webdriver/util/WebDriverUtils.java
			...
			public static WebElement getElementByLocator( final By locator ,WebDriver driver) {
				...
				while ( (System.currentTimeMillis() - startTime) < 91000 ) {
					logsObj.log( "Searching for element. Try number " + (tries++) );  
					try {
				...
				if ( found ) {
					logsObj.log("Found element after waiting for " + totalTime + " milliseconds." );
				} else {
					logsObj.log( "Failed to find element after " + totalTime + " milliseconds." );
				}
				return we;
			}

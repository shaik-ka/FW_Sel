...
public class HTMLReporting_2014_up extends Base{
	
	//BufferedWriter main_bw=null;
	//String screenShotLoc="D:\\HTMLReporting\\Screenshots\\";
	DateUtil dateObj;
	FileUtil fileObj;
	private static HTMLReporting_2014_up HTMLReportingObj;
	...
	private HTMLReporting_2014_up(){};
	
	public static HTMLReporting_2014_up getHTMLReportingObject(){
		if(HTMLReportingObj==null){
			HTMLReportingObj=new HTMLReporting_2014_up();
		}
	...
	private BufferedWriter fetchFile(String filefullPath){	
		...
		try{
			...
			t.printStackTrace();
			//System.out.println("fetchFile:-Unable to fetch/create the HTML file"+filefullPath+" due to error-"+t);
	...
	try{
		...
		t.printStackTrace();
		//System.out.println("closingTags:-Unable to close the tags of test summary results page due to error-"+t);
	...
	private BufferedWriter htmldoctype(String filefullPath,String stylesheetPath){
		...
		try{
			...
			t.printStackTrace();
			//System.out.println("htmldoctype:-Unable to map document type and stylesheet for the "+filefullPath+" due to error");
	...
	private BufferedWriter htmlstylesheet(String filefullPath,String title){
		...
		try{
			...
			t.printStackTrace();
			//System.out.println("htmlstylesheet:-Unable to stylesheet "+initiazlized_styleSheet+"for the "+filefullPath+" due to error");
	...
	private BufferedWriter includeSuitesMenu(BufferedWriter bw,LinkedHashMap<String,String>suitesList){
		...
		try{
			...
			bw.append("<ul> \n");
            //Enumeration<String>  suites1=suitesList.keys();
			...
			t.printStackTrace();
			//System.out.println("includeSuitesMenu:-Unable to include the suites in the summary page due to error-"+t);
	...
	private BufferedWriter TestResultSummaryTableAndGraph(BufferedWriter bw,String url){
		try{
			...
			t.printStackTrace();
			//System.out.println("TestResultSummaryTableAndGraph:-Unable to create Test Result Summary table due to error-"+t);
	...
	private BufferedWriter TestSummaryTableHead(BufferedWriter bw){
		try{
			...
			bw.append("<tr>");
			bw.append("<th style=\""+style_width1+"\"></th>");
			...
			t.printStackTrace();
			//System.out.println("TestSummaryTableHead:-Unable to publish the Head of Test Summary table head due to error-"+t);
	...
	public BufferedWriter logTestStatus(BufferedWriter bw,String scenarioName,String testcaseName,String testStatus,String test_srtime,String test_endtime){
		try{
			...
			t.printStackTrace();
			//System.out.println("logTestStatus:-Unable to publish the td for test case-"+testcaseName+" due to error-"+t);
	...
	private BufferedWriter SuiteSummaryTableHead(BufferedWriter bw,String loc_screenShot){
		...
		//	bw.append("<th class=\""+class_head+"\">Screenshot</th>");
			...
			t.printStackTrace();
			System.out.println("SuiteSummaryTableHead:-Unable to create the Suite Summary Table due to error-"+t);
	...
	public BufferedWriter logTestStepDetails(WebDriver driver,BufferedWriter bw,String suiteName,String scenarioName,String TestcaseRefID,String testcaseName,String browserName,String browserVersion,String environment,String step,String stepDescription,String stepStatus){
		try{
			...
			t.printStackTrace();
			System.out.println("logTestStepDetails:-Unable to publish the td for test case-"+testcaseName+" due to error-"+t);
		}
	...
	public  String UpdateHTMLTestSummary(String filePath) {
		...
		try {
			...
			//System.out.println("UpdateHTMLTestSummary:- Unable to update the Execution summary details in Test Summary page due to error-"+e);
	 ...
	 */
		void copyReqFolder(String resultsFolder){
			String sourceFile_styles=System.getProperty("user.dir")+"//HTMLReportingFiles_1.0//csssheets";

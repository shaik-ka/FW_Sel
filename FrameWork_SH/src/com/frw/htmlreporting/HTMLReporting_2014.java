...
public class HTMLReporting_2014 extends Base{
	
	//BufferedWriter main_bw=null;
	//String screenShotLoc="D:\\HTMLReporting\\Screenshots\\";
	DateUtil dateObj;
	FileUtil fileObj;
	private static HTMLReporting_2014 HTMLReportingObj;
	...
	private static String prv_scenarioName="";
	...
	private HTMLReporting_2014(){};
	
	public static HTMLReporting_2014 getHTMLReportingObject(){
		if(HTMLReportingObj==null){
			HTMLReportingObj=new HTMLReporting_2014();
		}
	...
	private BufferedWriter fetchFile(String filefullPath){	
		...
		try{
			...
			t.printStackTrace();
			//log("fetchFile:-Unable to fetch/create the HTML file"+filefullPath+" due to error-"+t);
	...
	try{
		...
		t.printStackTrace();
		//log("closingTags:-Unable to close the tags of test summary results page due to error-"+t);
	...
	private BufferedWriter htmldoctype(String filefullPath,String stylesheetPath){
		...
		try{
			...
			t.printStackTrace();
			//log("htmldoctype:-Unable to map document type and stylesheet for the "+filefullPath+" due to error");
	 ...
	 * @return
	 */
	private BufferedWriter htmlstylesheet(String filefullPath,String title){
		BufferedWriter bw=null;
		try{
			...
			t.printStackTrace();
			//log("htmlstylesheet:-Unable to stylesheet "+initiazlized_styleSheet+"for the "+filefullPath+" due to error");
	 ...
	 * @return
	 */
	private BufferedWriter createHTMLAndMapStyleSheet(String filefullPath,String title){
		BufferedWriter bw=null;
		bw=htmlstylesheet(filefullPath,title);
	 ...
	 * @return
	 */
	private BufferedWriter includeSuitesMenu(BufferedWriter bw,Hashtable<String,String>suitesList){
		
		
		try{
			
			bw.append("<div class=\""+class_menu+"\"> \n");
			bw.append("<ul> \n");
			...
			t.printStackTrace();
			//log("includeSuitesMenu:-Unable to include the suites in the summary page due to error-"+t);
...
private BufferedWriter includeSuitesMenu(BufferedWriter bw,LinkedHashMap<String,String>suitesList){
		...
		try{
			...
			for (String key : suitesList.keySet()) {
				bw.append("<li class=\""+class_menu+"\"><a href=\""+key+".html\">"+key+"</a></li>");
	...
	private BufferedWriter TestResultSummaryTableAndGraph(BufferedWriter bw,String url){
		try{
			//bw.append("<table id=\""+id_testresultsummary+"\" align=\"right\">");
			bw.append("<table id=\""+id_testresultsummary+"\" align=\"right\" style=\"width: 60%;\">");
			...
			bw.append("</tr>");
			bw.append("</table>");
		//	bw.append("<div id=\"piechart_3d\" style=\"width: 530px; height: 160px;\" align =\"left\"></div>");
			bw.append("<div id=\"piechart_3d\" style=\"width: 25%; height: 175px;\" align =\"left\"></div>");
			
		}catch(Throwable t){
			t.printStackTrace();
			//log("TestResultSummaryTableAndGraph:-Unable to create Test Result Summary table due to error-"+t);
	...
	private BufferedWriter TestSummaryTableHead(BufferedWriter bw){
		try{
			...
			bw.append("<tr>");
			bw.append("<th style=\""+style_width2+"\"></th>");
			...
			t.printStackTrace();
			//log("TestSummaryTableHead:-Unable to publish the Head of Test Summary table head due to error-"+t);
	...
	public BufferedWriter logTestStatus(BufferedWriter bw,String scenarioName,String testcaseName,String testStatus,String test_srtime,String test_endtime){
		try{
			if(!prv_scenarioName.equalsIgnoreCase(scenarioName)){
				scenarioCount++;
			}
			if(testStatus.equalsIgnoreCase("Pass")){
				passCount++;
				bw.append("<tr>");
				//bw.append("<td><img src=\"images\\success.png\"/></td>");
				bw.append("<td><div id=\""+id_imgsuccess+"\"/></td>");

			}else{
				failCount++;
				bw.append("<tr class=\"alt\">");
				//bw.append("<td><img src=\"images\\fail.png\"/></td>");
				bw.append("<td><div id=\""+id_imgfailure+"\"/></td>");
			}
			...
			t.printStackTrace();
			//log("logTestStatus:-Unable to publish the td for test case-"+testcaseName+" due to error-"+t);
	 ...
	 * @return
	 */
	public BufferedWriter createHTMLForTestSummary(String testSummaryFilePath,String resultsFolder,Boolean isRemoteDriverExecution,String testRepositoryURL, String title,Hashtable<String,String>suites,String AUT_Url){
		...
		BufferedWriter summary_bfw=createHTMLAndMapStyleSheet(testSummaryFilePath,title);
		includeSuitesMenu(summary_bfw,suites);
	 ...
	 * @return
	 */
	public BufferedWriter createHTMLForSuiteExecution(String suiteFilePath,String title,String screensFolderPath){
	
		BufferedWriter suite_bfw=createHTMLAndMapStyleSheet(suiteFilePath,title);
	...
	private BufferedWriter SuiteSummaryTableHead(BufferedWriter bw,String loc_screenShot){
		...
		//	bw.append("<th class=\""+class_head+"\">Screenshot</th>");
			...
			t.printStackTrace();
			//log("SuiteSummaryTableHead:-Unable to create the Suite Summary Table due to error-"+t);
	...
	public BufferedWriter logTestStepDetails(WebDriver driver,BufferedWriter bw,String suiteName,String scenarioName,String TestcaseRefID,String testcaseName,String browserName,String browserVersion,String environment,String step,String stepDescription,String stepStatus){
		try{
			/* commented on Nov 24 as warning has marked in red
	...
	public  String UpdateHTMLTestSummary(String filePath) {
		...
		try {
			...
			contentBuilder.replace(cnt1,cnt1+replace_totalScenarioExecuted.length(),Integer.toString(scenarioCount));			   
			...
			e.printStackTrace();
			//log("UpdateHTMLTestSummary:- Unable to update the Execution summary details in Test Summary page due to error-"+e);
		...
		try{
			...
			//log("UpdateHTMLTestSummary-Error: " + e.getMessage());
			logsObj.logError(" Unable to update HTML Test Summary Page due to error",e);
		}
	 ...
	 */
		void copyReqFolder(String resultsFolder){
			String sourceFile_styles=System.getProperty("user.dir")+"//HTMLReportingFiles//csssheets";
			String destFile_styles=resultsFolder+"//csssheets";

			String sourceFile_images=System.getProperty("user.dir")+"//HTMLReportingFiles//images";

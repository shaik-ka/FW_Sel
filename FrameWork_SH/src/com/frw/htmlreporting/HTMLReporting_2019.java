...
public class HTMLReporting_2019 extends Base{
	...
	HashMap<String,String> map_passTC=new HashMap<String, String>();
	HashMap<String,String> map_failTC=new HashMap<String, String>();
	HashMap<String,String> map_scenarios=new HashMap<String, String>();
	...
	String class_wrappable="wrappable";
	String style_width1="width:1%";
	String style_width3="width:3%";
	String style_widthAuto="width:auto";
	String style_width2="width:2%";
	String style_widthp01="width:0.01%";
	String style_width10="width:10%";
	String style_width20="width:20%";
	...
	private HTMLReporting_2019(){};

	public static HTMLReporting_2019 getHTMLReportingObject(){
		if(HTMLReportingObj==null){
			HTMLReportingObj=new HTMLReporting_2019();
		}
	...
	private BufferedWriter TestSummaryTableHead(BufferedWriter bw){
		try{
			...
			bw.append("<th style=\""+style_width1+"\">Start Time</th>");
			bw.append("<th style=\""+style_width1+"\">End Time</th>");
			//bw.append("<th class=\""+class_head+"\">Duration(HH:MM:SS)</th>");
			bw.append("<th style=\""+style_widthp01+"\">Duration (HH:MM:SS)</th>");
			bw.append("</tr>");
	...
	private BufferedWriter tdTestSummaryTemplate(BufferedWriter bw,String scenarioName,String testcaseName,String testStatus,String test_strtime,String test_endtime,String duration) throws Throwable{
		...
		bw.append("<td  class=\""+class_wrappable+"\" >"+scenarioName+"</td>");
		bw.append("<td class=\""+class_wrappable+"\">"+testcaseName+"</td>");
	 ...
	 * @return
	 */
	public BufferedWriter createHTMLForTestSummary(String testSummaryFilePath,String resultsFolder,String captureScreenTool,String testRepositoryURL, String title,LinkedHashMap<String, String> suites,String AUT_Url){
		captureScreenWith=captureScreenTool;
	...
	private BufferedWriter SuiteSummaryTableHead(BufferedWriter bw,String loc_screenShot){
		...
		try{
			...
			bw.append("<th style=\""+style_width1+"\">Stp</th>");
			bw.append("<th >Suite/Module Name</th>");
			bw.append("<th style=\""+style_width15+"\">ScenarioName/ID</th>");
			bw.append("<th style=\""+style_width2+"\">Test RefID</th>");			
			bw.append("<th style=\""+style_width20+"\">Testcase Name</th>");
			bw.append("<th style=\""+style_width1+"\">Browser Name   /  Version</th>");
			bw.append("<th style=\""+style_width1+"\">OS</th>");
			bw.append("<th style=\""+style_width25+"\">Step/Action Details</th>");
			bw.append("<th style=\""+style_width25+"\">Step Description</th>");
			//bw.append("<th class=\""+class_head+"\">Step Description</th>");			
			bw.append("<th >Status</th>");
			//	bw.append("<th class=\""+class_head+"\">Screenshot</th>");
			bw.append("<th >Executed on</th>");
			//bw.append("<th class=\""+class_head+"\">Screenshot</th>");
			bw.append("<th >Screenshot</th>");
			bw.append("</tr>");
	...
	private BufferedWriter tdTestStepTemplate(WebDriver driver,BufferedWriter bw,String suiteName,String scenarioName,String TestcaseRefID ,String testcasename,String browserName,String browserVersion,String environment,String step,String stepDescription,String status) throws Throwable{
		...
		bw.append("<td  class=\""+class_wrappable+"\" >"+testcasename+"</td>");
		bw.append("<td>"+browserName+"/"+browserVersion+"</td>");
		//bw.append("<td>"+browserVersion+"</td>");
		bw.append("<td>"+environment+"</td>");
		bw.append("<td  class=\""+class_wrappable+"\" >"+step+"</td>");
		bw.append("<td  class=\""+class_wrappable+"\" >"+stepDescription+"</td>");
		bw.append("<td>"+status+"</td>");		
		bw.append("<td>"+dateObj.getFormattedDate()+"</td>");
		if(status.equalsIgnoreCase("Fail")){

			String RESULT_FOLDER_LOCATION=screenShotLoc;
			String screenFormat="PNG";
			...
			String failureImageFileNameRelPath = "..\\Screenshots\\"+failureImageFileName;
			captureObj=CaptureScreenUtil.getCaptureScreenUtil();
			//captureObj=captureObj.getCaptureScreenUtil();
			if(captureScreenWith.equalsIgnoreCase("WEBDRIVER")){
			...
			else if(captureScreenWith.equalsIgnoreCase("ROBO")){
			...
			bw.append("<td><a href=\""+failureImageFileNameRelPath+"\"><img alt=\""+failureImageFileName+"\" src=\""+failureImageFileNamePath+"\" width=\"80\" height=\"40\" /></a> </td>");
		}else{
			bw.append("<td></td>");
		}
	...
	public BufferedWriter logTestStepDetails(WebDriver driver,BufferedWriter bw,String suiteName,String scenarioName,String TestcaseRefID,String testcaseName,String browserName,String browserVersion,String environment,String step,String stepDescription,String stepStatus){
		try{
			if(stepStatus.equalsIgnoreCase("Fail")){
				bw.append("<tr class=\"alt\">");
				map_failTC.put(TestcaseRefID, TestcaseRefID);
			}else{				
				bw.append("<tr>");
				map_passTC.put(TestcaseRefID, TestcaseRefID);
			}
			map_scenarios.put(scenarioName, scenarioName);
			//Nov 3
	...
	public  String UpdateHTMLTestSummary(String filePath) {
		...
		/*	Set<String> failTCs=map_failTC.keySet();
			...
			contentBuilder.replace(cnt3,cnt3+replace_testsPassed.length(),String.valueOf(totalPass));

			int cnt4 = contentBuilder.indexOf(replace_testsFailed);
			contentBuilder.replace(cnt4,cnt4+replace_testsFailed.length(),String.valueOf(totalFail));
			...
			contentBuilder.replace(cnt6,cnt6+replace_graph_testsPassed.length(),String.valueOf(totalPass));
			System.out.println("Graph pass count is "+String.valueOf(map_passTC.keySet().size()));
			int cnt7 = contentBuilder.indexOf(replace_graph_testsFailed);
			contentBuilder.replace(cnt7,cnt7+replace_graph_testsFailed.length(),String.valueOf(totalFail));
			System.out.println("Graph fail count is "+String.valueOf(map_failTC.keySet().size()));
